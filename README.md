...
To protect against tampering, Absoluta adds HMAC (Hash-based Message Authentication Code) to each encrypted message:

1. After encryption, the system computes HMAC-SHA256 using the seed as the key
2. The HMAC is appended to the ciphertext separated by a pipe character (`|`)
3. During decryption, the HMAC is verified before any decryption occurs
4. If the HMAC doesn't match, the system rejects the message as tampered

This provides **integrity protection** — any modification to the ciphertext will be detected and the decryption will be aborted.
...
- **Integrity protection** via HMAC-SHA256 authentication
